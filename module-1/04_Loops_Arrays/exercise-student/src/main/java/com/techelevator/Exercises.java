...
public class Exercises {
	...
	public boolean sameFirstLast(int[] nums) {
		...
		return (nums.length>=1 && (nums[nums.length-1] ==nums[0])) ? true : false;
	}
	...
	public int[] maxEnd3(int[] nums) {
		...
		for (int i = 0; i <3; i++) {
			returnArray[i]=(nums[0]>nums[nums.length-1])?nums[0]:nums[nums.length-1];
			}
		return(returnArray);
	}
	...
	public int sum2(int[] nums) {
		return(nums.length==0)?0:(nums.length<2)?(nums[0]):(nums[0]+nums[1]);
	}
